  ...
  createModal() {
    ...
    switch (this.type) {
      ...
      case "info": {
        ...
        rules.innerText =
          "RULES: Guess a letter in the word; if it's in the word, all the spots that match that letter fill in. For example, if the word was ERROR, then, when no letters have been guessed, it would be _ _ _ _ _ . If the letter R is guessed, it becomes  _ R R _ R . If the player guesses wrong, a piece is added to the hangman person. When the entire human is built, the player loses.";
        const note = createNode("p", ["modal__text"]);
        note.innerText =
          "IMPORTANT: App works with physical keyboard. Please, make sure you are using EN keyboard. The word consists of letters A-Z of English alphabet only.";
