  ...
  createModal() {
    ...
    switch(this.type) {
      ...
      case 'info': {
        ...
        rules.innerText = "RULES: Guess a letter in the word; if it's in the word, all the spots that match that letter fill in. For example, if the word was ABYSSAL, then, when no letters have been guessed, it would be _ _ _ _ _ _ _ . If the letter A is guessed, it becomes A _ _ _ _ A _ . If the player guesses wrong, a piece is added to the hangman person. When the entire human is built, the player loses."
        const note = createNode('p', "modal__text");
        note.innerText = "NB: App works with physical keyboard. Please, make sure you are using EN keyboard. The word consist of letters A-Z of English alphabet only.";
        const linkMDN = createNode('a', ["modal__link"], {'target':'_blank', 'href':'https://developer.mozilla.org/en-US/docs/Glossary'}, 'MDN web docs glossary');
        const linkLearnJS = createNode('a', ["modal__link"], {'target':'_blank','href':'https://javascript.info/'}, 'javascript.info');
        const sources = createNode('p', "modal__text");
        sources.append('in the game you are encouraged to guess for words on the topic related to web development. You can find most of definitions in ', linkMDN, ' and ', linkLearnJS);
