...
export function isTypeOf<T extends ObjectType>(object: unknown, template: T) {
...
export function assertsObjectIsTypeOf<T extends ObjectType>(object: unknown, template: T): asserts object is T {
  if (!isTypeOf(object, template)) throw new Error("Invalid data shape!");
}

export function isArrayOfType(array: unknown[], type: "string" | "boolean" | "function" | "object" | "number") {
...
export function assertsArrayOfStrings(array: unknown[]): asserts array is string[] {
  if (!isArrayOfType(array, "string")) throw new Error("Invalid data shape!");
}

export function isArrayOfObjectsOfType<T extends ObjectType>(array: unknown, template: T) {
...
export function assertsArrayOfObjectIsTypeOf<T extends ObjectType>(array: unknown, template: T): asserts array is T[] {
  if (!isArrayOfObjectsOfType(array, template)) throw new Error("Invalid data shape!");
}
