import { IDriveStatusResponse } from "../../../types/response-interfaces";
...
async function getCarRaceResult(car: IRaceParticipants, controller: AbortController): Promise<IRaceParticipants> {
  if (!car.raceParams) return Promise.reject(new Error(`not found`));
  if (car.raceParams.error) return Promise.reject(new Error(`not found`));
  ...
  if (carResult.status === 500) car.component.updateCarStateLabel(LABELS.broken);
  if (carResult.status === 404) car.component.updateCarStateLabel(LABELS.broken);
