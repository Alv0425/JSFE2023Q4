...
export class Game extends Base {
  ...
  checkGameState() {
    const isWin = () => {
      for (let i = 0; i < this.currentGame.size; i++) {
       for (let j = 0; j < this.currentGame.size; j++) {
        if (this.currentGame.field[i][j] === 0 && this.cells[i][j].classList.contains('black')) return false;
        if (this.currentGame.field[i][j] === 1 && !this.cells[i][j].classList.contains('black')) return false;
       }
      }
      return true;
    }
    ...
    if (isWin()) {
      ...
      textWin.textContent = `Great! You have solved the nonogram in ${Math.round(this.timer)} seconds!"`;
